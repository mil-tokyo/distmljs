...
async def collect_random_data():
    ...
    for client_id in sv.worker_ids:
        await sv.twice2(sv.send_msg_for_collecting_random_samples)(client_id)
    ...
    while replay_buffer.size < int(opt.start_timesteps):
        ...
        if isinstance(event, KakiageServerWSReceiveEvent):
            ...
            if event.message['type'] == 'worker':
                await sv.twice2(sv.send_msg_for_collecting_random_samples)(event.client_id)
...
async def get_gradient():
    ...
    for c, learner_id in enumerate(sv.learner_ids):
        ...
        await sv.twice2(sv.send_msg_to_learner_for_collecting_grads)(learner_id, dataset_item_id)
    ...
    while len(complete_learner_grad_ids) < len(sv.learner_ids):
        ...
        if isinstance(event, KakiageServerWSReceiveEvent):
            ...
            # learners
                ...
                print(
                    f"[{len(complete_learner_grad_ids)}/{len(sv.learner_ids)}] Got connected by learner: {event.client_id}.")
...
async def main():
    ...
    for trials_id in range(opt.n_trials):
        ...
        print(
            f"Model: {opt.model_name}, dataset: {opt.dataset_name}, batch size: {opt.batch_size}")
        ...
        if trials_id > 0:
            for client_id in previous_worker_ids:
                await sv.twice2(sv.send_msg_reload)(client_id)
        ...
        if opt.continue_train and trials_id == opt.start_trial_id:
            start_time = float(
                str(list(root_dir.glob("start_*"))[0]).split("_")[-1])
        else:
            if opt.save_weights:
        ...
        if opt.continue_train and trials_id == opt.start_trial_id:
            save_path = sorted(
                list(root_dir.glob(f"*{opt.start_iter}*.pkl")))[-1]
        ...
        while training.iter < opt.train_step_num:
            ...
            with torch.no_grad():
                ...
                if len(connecting_learners) < len(sv.init_learners):
                    ...
                    missing_learners = sv.init_learners - \
                        set(connecting_learners.keys())
                    ...
                    for i, client_id in enumerate(connecting_actors):
                        if i < len(missing_learners):
                            print(
                                f"reassignment: new learner is '{client_id}'")
                ...
                if training.iter == 1 or training.iter % opt.test_freq == 0 or training.iter == opt.train_step_num:
                    ...
                    # update test plot
                    stats = training.update_test_log(
                        replay_buffer.size, save_reward, elapsed_time)
                    last_stats = {key: value[-1]
                    ...
                    for client_id in list(sv.tester_ids):
                        if client_id in sv.init_learners:
                            ...
                            sv.actor_ids.add(client_id)
                            await sv.twice2(sv.send_msg_to_actor_for_collecting_samples)(client_id)
                ...
                if opt.save_weights:
                    if training.iter == 1 or training.iter % opt.save_freq == 0 or training.iter == opt.train_step_num:
                        ...
                        # save global weight
                        save_path = root_dir / \
                            f"{training.iter:08}iter_{last_stats['reward']:.3f}reward"

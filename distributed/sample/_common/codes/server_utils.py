        ...
        async def try_3_times(client_id, *args):
            try:
                ...
                try:
                    ...
                    time.sleep(sleep_time)
                    print(
                        f"Failed to send msg to client: {client_id}. Try again.")
                    try:
                        await async_func(client_id, *args)
                    except KeyError:
                        pass
...
class ReplayBuffer(object):
    ...
    def clear(self):
        for name, dim in self.val_names_dims.items():
            setattr(self, name, np.zeros(
                (self.max_size, dim), dtype=np.float32))
    ...
    def add(self, loaded_buffer):
        ...
        assert len(
            input_length) == 1, f'Buffer size must be same among attributes. Got {", ".join(map(str, input_length))}.'
        ...
        slices = [slice(self.ptr, min(end_ptr, self.max_size)),
                  slice(0, max(0, end_ptr - self.max_size))]
        ...
        # update priority
        ind, priority = np.concatenate([np.arange(self.max_size)[
                                       idx] for idx in slices]), loaded_buffer["td"].reshape(input_length)
    ...
    def sample(self, batch_size, n_classes):
        ...
        ret = dict([(name, getattr(self, name)[ind])
                    for name in self.val_names_dims.keys()])

        # すごく一時的な実装 # そのうちやめる
        ret["action_mask"] = np.eye(
            n_classes)[ret["action"].squeeze().astype(np.int64)].astype(np.float32)
        del ret["action"]

...
from kakiage import tensor_serializer
...
def train_eval(model, bptt, ntokens, device, train_data, val_data):
    ...
    for epoch in range(1, epochs + 1):
        ...
        train(model, train_data, optimizer, scheduler,
              epoch, bptt, ntokens, device, criterion)
...
def train(model: nn.Module, train_data, optimizer, scheduler, epoch, bptt, ntokens, device, criterion) -> None:
...
def evaluate(model: nn.Module, eval_data: Tensor, bptt, ntokens, device, criterion) -> float:
...
def make_sample_grad(model, eval_data, bptt, ntokens, device, criterion):
    ...
    batch_data = {"data": data.detach().cpu().to(torch.int32).numpy(), "targets": targets.detach(
    ).cpu().to(torch.int32).numpy(), "src_mask": src_mask.detach().cpu().numpy()}
...
def map_weight_to_kakiage(torch_weights):
    new_dict = {}
...
def main():
    print("This sample trains transformer model with PyTorch and saves data to check correctness of gradient computation in kakiage.")
    ...
    model = TransformerModel(ntokens, emsize, nhead,
                             d_hid, nlayers, dropout).to(device)
    best_model = train_eval(model, bptt, ntokens, device, train_data, val_data)

    criterion = nn.CrossEntropyLoss()

    best_model_weights = best_model.state_dict()
    grads, sample_batch = make_sample_grad(model, test_data, bptt,
                                           ntokens, device, criterion)
    print("saving trained model to pytorch_trained_model.pt")
    torch.save(best_model_weights, os.path.join(
        out_dir, "pytorch_trained_model.pt"))
    print("converting weight for kakiage to pytorch_trained_weight.bin")
    with open(os.path.join(out_dir, "pytorch_trained_weight.bin"), "wb") as f:
        f.write(tensor_serializer.serialize_tensors_to_bytes(
            map_weight_to_kakiage(best_model_weights)))
    ...
    with open(os.path.join(out_dir, "pytorch_trained_grad.bin"), "wb") as f:
        f.write(tensor_serializer.serialize_tensors_to_bytes(
            map_weight_to_kakiage(grads)))
    print("open http://localhost:8080/sample/transformer/output/gradient_check.html to run gradient check.")

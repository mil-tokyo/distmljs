...
def serialize_save(dataset, path):
    ...
    for i in range(len(dataset)):
        ...
        # int64 -> int32 (distmljs's tensor must be float32/int32/uint8/bool)
        tostack[1].append(np.int32(target))

    # serialize tensor into distmljs's format
...
def main():
    ...
    test_dataset = datasets.MNIST("../pytorch_data", train=False, transform=transform)
    serialize_save(
        train_dataset, os.path.join(output_dir, "mnist_preprocessed_flatten_train.bin")
    )
    serialize_save(
        test_dataset, os.path.join(output_dir, "mnist_preprocessed_flatten_test.bin")
    )
